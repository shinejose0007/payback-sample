...
import json
import datetime
from scripts.upload_model import upload_artifact_if_configured
...
def load_data(path=DATA_PATH):
    df = pd.read_csv(path)
    # drop non-numeric / date columns used for tracking
...
def create_synthetic_data(path=DATA_PATH, n=1000, random_state=42):
    ...
    print(f"Written synthetic data to {path}")

def write_manifest(model_metrics: dict, path=MANIFEST_PATH):
...
def train_and_save():
    ...
    write_manifest({'mse': mse})
    upload_artifact_if_configured(MODEL_PATH, manifest_path=MANIFEST_PATH)
